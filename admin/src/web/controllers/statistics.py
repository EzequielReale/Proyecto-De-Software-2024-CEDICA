...
def index():
    ...
    if (
        not check_permission(session, "reg_pagos_index")
        or not check_permission(session, "reg_cobros_index")
        or not check_permission(session, "team_index")
        or not check_permission(session, "jya_index")
    ):
        return unauthorized()
...
def balance():
    ...
    if not check_permission(session, "reg_pagos_index") or not check_permission(
        session, "reg_cobros_index"
    ):
        return unauthorized()
...
@login_required
def age():
    """Genera un gráfico de torta con el porcentaje de jinetes/amazonas en el rango de edad
    de 3 a 18 años, 19 a 35 años, 36 a 50 años, 51 a 65 años y más de 65 años"""
...
def balance_report():
    ...
    return render_template(
        "statistics/balance_report.html",
        incomes=incomes,
        outflows=outflows,
        filters=filters,
    )
...
@bp.get("/balance_graphic")
@login_required
def balance_graphic():
    """Renderiza la página del gráfico de balance"""
    if not check_permission(session, "reg_pagos_index") or not check_permission(
        session, "reg_cobros_index"
    ):
        return unauthorized()

    return render_template("statistics/balance_graphic.html")


@bp.get("/job_graphic")
@login_required
def job_graphic():
    """Renderiza la página del gráfico de puestos laborales"""
    if not check_permission(session, "team_index"):
        return unauthorized()

    return render_template("statistics/job_graphic.html")


@bp.get("/age_graphic")
@login_required
def age_graphic():
    """Renderiza la página del gráfico de rango de edades"""
    if not check_permission(session, "jya_index"):
        return unauthorized()

    return render_template("statistics/age_graphic.html")
