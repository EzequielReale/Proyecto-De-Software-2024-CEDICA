<template>
  <section id="articles" style="padding-top: 125px">
    <div v-if="articles.length === 0" class="loading">Cargando artículos...</div>
    <div v-else class="articles-container">
      <h1 class="fw-bold mb-5">Noticias y actividades</h1>
      ...
      <div class="load-more-container">
        <button @click="loadMore" v-if="hasMore" class="load-more-button">Cargar más</button>
      </div>
    </div>
  </section>
</template>
...
const fetchArticles = async () => {
  try {
    const response = await axios.get(
      `https://admin-grupo04.proyecto2024.linti.unlp.edu.ar/api/articles?page=${page.value}&per_page=${perPage}`,
    )
